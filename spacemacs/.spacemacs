  ...
  (setq-default
   ...
   '(
     ...
     (spacemacs-project :variables
                        projectile-project-search-path '(("~/git/" . 2) ("~/unversioned/" . 1) )
...
before packages are loaded."
  ...
  (with-eval-after-load 'terminal-here
    (setopt terminal-here-linux-terminal-command 'alacritty))

  (with-eval-after-load 'gptel
    (setq
     gptel-model 'gemma2:9b
     gptel-backend (gptel-make-ollama "Ollama"
                                      :host "localhost:11434"
                                      :stream t
                                      :models '(gemma2:9b llama3:8b mistral:7b)))
    ;; Optional: Custom keybindings or system prompt
    (gptel-make-org-minibuffer-client "Ollama Chat"))

  (with-eval-after-load 'ellama
    (require 'llm-ollama)
    (setopt ellama-provider
            (make-llm-ollama
             :chat-model "gemma2:9b" ;; For conversation
             :embedding-model "mxbai-embed-large" ;; For search/RAG
             :default-chat-non-standard-params '(("num_ctx" . 8192))))
    ;; Why DeepSeek-Coder-V2
    ;;  - Broad coverage: Strong on JS/TS (Node/React), Rust (async/safe), Scala (FP/JVM), Java (enterprise), Haskell/PureScript (type-safe FP)—no niche gaps.
    ;;  - Ollama tags: deepseek-coder-v2:16b (RTX 4070-friendly), deepseek-coder-v2-lite (faster, 4-6 GB VRAM).
    ;;  - 2026 consensus: Beats Qwen2.5-Coder, CodeLlama, StarCoder2 in multi-lang evals.
    (setopt ellama-coding-provider
            (make-llm-ollama
             :chat-model "deepseek-coder-v2:16b"
             :embedding-model "nomic-embed-text"
             :default-chat-non-standard-params '(("num_ctx" . 32768)))) ;; 32K Tokens, ~6-8 GB VRAM used

    (setopt ellama-providers
            '(("gemma2" . (make-llm-ollama :chat-model "gemma2:9b"))
              ("deepseek-coder-v2" . (make-llm-ollama :chat-model "deepseek-coder-v2:16b"))
              ("mistral" . (make-llm-ollama :chat-model "mistral:7b"))
              ("llama3" . (make-llm-ollama :chat-model "llama3:8b"))))

    (setopt ellama-generate-commit-message-template
            "<INSTRUCTIONS>
               You are a professional developer. Write commit message STRICTLY
...
This function is called at the very end of Spacemacs initialization."
  (custom-set-variables
   ...
   '(package-selected-packages
     '(ace-link add-node-modules-path aggressive-indent all-the-icons attrap
                auto-compile auto-highlight-symbol auto-yasnippet
                avy-jump-helm-line bui centered-cursor-mode clean-aindent-mode
                cmm-mode code-cells code-review column-enforce-mode company-cabal
                company-nixos-options company-web cython-mode dante dap-mode
                define-word devdocs diminish dired-quick-sort disable-mouse
                dotenv-mode drag-stuff dumb-jump eat edit-indirect elisp-def
                elisp-demos elisp-slime-nav ellama emmet-mode emr esh-help
                ...
                flyspell-correct flyspell-correct-helm ggtags gh-md git-link
                git-messenger git-modes git-timemachine gitignore-templates
                gnuplot golden-ratio google-translate gptel grizzl haml-mode
